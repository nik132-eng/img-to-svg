...
export default function SharedSvgPage() {
  ...
  const parseBulkSvgContent = (svgContent: string, fileName: string): ParsedSvgItem[] => {
    try {
  ...
  useEffect(() => {
    const fetchSharedSvg = async () => {
      try {
        ...
        const parsed = parseBulkSvgContent(data.svgContent, data.fileName);
  ...
  const handleDownload = () => {
    if (!svgData || parsedSvgs.length === 0) return;
          ...
          <div className="lg:col-span-2">
            <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl border border-white/40 p-6">
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-600">
                  {isBulkSvg 
                    ...
                    : `File: ${svgData.fileName.replace(/\.[^/.]+$/, '.svg')}`
                  }
                </p>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-4 min-h-96 flex items-center justify-center overflow-hidden">
                <div 
                  dangerouslySetInnerHTML={{ __html: parsedSvgs[selectedSvgIndex]?.svgContent || svgData.svgContent }}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-xl border border-white/40 p-6">
              ...
              <div className="space-y-3">
                <div>
                  <p className="text-sm text-gray-600">
                    {isBulkSvg ? 'Current SVG Name' : 'Original Name'}
                  </p>
                  <p className="text-sm font-medium text-gray-800">
                    {isBulkSvg 
                      ? parsedSvgs[selectedSvgIndex]?.fileName || 'Unknown'
                      : svgData.fileName
                    }
                  </p>
                </div>
                {isBulkSvg && (
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-800">{parsedSvgs.length} SVGs</p>
                  </div>
                )}
                <div>
